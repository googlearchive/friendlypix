...
- (id)initWithUser:(FPUser *)aUser {
    ...
    if (self) {
        ...
        if (!aUser) {
            [NSException raise:NSInvalidArgumentException format:@"FPAccountViewController init exception: user cannot be nil"];
        }
...
//    }
    ...
    UIImageView *profilePictureImageView = [[UIImageView alloc] initWithFrame:CGRectMake( 94.0f, 38.0f, 132.0f, 132.0f)];
    ...
    [profilePictureImageView setCircleImageWithURL:[user profilePictureURL] placeholderImage:[UIImage imageNamed:@"ProfilePlaceholder"] borderWidth:2];
    ...
    [photoCountLabel setText:[NSString stringWithFormat:@"%lu photo%@", feedCount, feedCount==1?@"":@"s"]];

    unsigned long followersCount = [followers count];
    [followerCountLabel setText:[NSString stringWithFormat:@"%lu follower%@", followersCount, followersCount==1?@"":@"s"]];

    [followingCountLabel setText:[NSString stringWithFormat:@"%lu following", followingCount]];
  ...
  if (![self.user.userID isEqualToString:[FPAppState sharedInstance].currentUser.userID]) {
    ...
    if ([followers containsObject:self.user.userID]) {
...
- (void)followButtonAction:(id)sender {
  ...
  [[super.ref childByAppendingPath: [NSString stringWithFormat:@"users/%@/followers/%@", user.userID, [FPAppState sharedInstance].currentUser.userID]] setValue:[NSNumber numberWithBool:YES]];
  [[super.ref childByAppendingPath: [NSString stringWithFormat:@"users/%@/following/%@", [FPAppState sharedInstance].currentUser.userID, user.userID]] setValue:[NSNumber numberWithBool:YES]];

  Firebase *myFeed = [super.ref childByAppendingPath:[NSString stringWithFormat:@"users/%@/feed", [FPAppState sharedInstance].currentUser.userID]];
  [[super.ref childByAppendingPath:[NSString stringWithFormat:@"users/%@/posts", user.userID]] observeSingleEventOfType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot) {
...
- (void)unfollowButtonAction:(id)sender {
  ...
  [[super.ref childByAppendingPath: [NSString stringWithFormat:@"users/%@/followers/%@", user.userID, [FPAppState sharedInstance].currentUser.userID]] setValue:[NSNumber numberWithBool:NO]];
  [[super.ref childByAppendingPath: [NSString stringWithFormat:@"users/%@/following/%@", [FPAppState sharedInstance].currentUser.userID, user.userID]] setValue:[NSNumber numberWithBool:NO]];
