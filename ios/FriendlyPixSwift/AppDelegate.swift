...
class AppDelegate: UIResponder, UIApplicationDelegate, GCMReceiverDelegate {
  ...
  func applicationDidBecomeActive( application: UIApplication) {
    GCMService.sharedInstance().connectWithHandler({
      ...
      if error != nil {
        ...
        FPAppState.sharedInstance.connectedToGcm = true
  ...
  func applicationDidEnterBackground(application: UIApplication) {
    ...
    FPAppState.sharedInstance.connectedToGcm = false
  }
  ...
  func application( application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken
    deviceToken: NSData ) {
      ...
      GGLInstanceID.sharedInstance().tokenWithAuthorizedEntity(FPAppState.sharedInstance.senderID,
  ...
  func application( application: UIApplication, didFailToRegisterForRemoteNotificationsWithError
    error: NSError ) {
      ...
      NSNotificationCenter.defaultCenter().postNotificationName(
        FPConstants.NotificationKeys.Registration, object: nil, userInfo: userInfo)
  }

  func registrationHandler(registrationToken: String!, error: NSError!) {
    if (registrationToken != nil) {
      FPAppState.sharedInstance.registrationToken = registrationToken;
  ...
  func application( application: UIApplication,
    didReceiveRemoteNotification userInfo: [NSObject : AnyObject]) {
      ...
      NSNotificationCenter.defaultCenter().postNotificationName(
        FPConstants.NotificationKeys.Message, object: nil, userInfo: userInfo)
  }
  ...
  func connectToFriendlyPing() {
    if FPAppState.sharedInstance.connectedToGcm && FPAppState.sharedInstance.signedIn &&
      FPAppState.sharedInstance.registrationToken != nil {
        subscribeToTopic()
    }
  }

  func subscribeToTopic() {
    if !FPAppState.sharedInstance.subscribed {
      GCMPubSub().subscribeWithToken(FPAppState.sharedInstance.registrationToken!,
        topic: FPConstants.GCMStrings.Topic, options: nil, handler: {
          (NSError error) -> Void in
          if (error != nil) {
            ...
            FPAppState.sharedInstance.subscribed = true
          }
      })
    }
  }
